...
class VocabularyChaptersQATests {
  ...
  Future<void> runAllTests() async {
    stdout.writeln('🧪 Starting Vocabulary Chapters QA Test Suite');
    stdout.writeln('=' * 60);
  ...
  Future<void> _setupTestUser() async {
    ...
    try {
      ...
      stdout.writeln('✅ Test user registered successfully');
    } catch (e) {
      stdout.writeln(
        'ℹ️  Test user might already exist, continuing with login...',
      );
    }
    ...
    stdout.writeln('✅ Test user authenticated successfully');
    stdout.writeln('   Token: ${authToken!.substring(0, 20)}...');
  ...
  Future<void> _testInitialState() async {
    ...
    stdout.writeln('✅ Initial state validation passed');
    stdout.writeln('   - Chapter 1 is unlocked: ✅');
    stdout.writeln('   - Total chapters: ${stats['totalChapters']}');
    stdout.writeln('   - Unlocked chapters: ${stats['unlockedChapters']}');
    stdout.writeln('   - Completed chapters: ${stats['completedChapters']}');
  }
  ...
  Future<void> _testProgressiveUnlocking() async {
    ...
    stdout.writeln('   Initial state:');
    stdout.writeln('   - Chapter 1 unlocked: ${chapter1['isUnlocked']}');
    stdout.writeln('   - Chapter 1 completed: ${chapter1['isCompleted']}');
    stdout.writeln('   - Chapter 2 unlocked: ${chapter2['isUnlocked']}');

    // Complete chapter 1 if not already completed
    if (!chapter1['isCompleted']) {
      stdout.writeln('   Completing chapter 1...');

      final completeResponse = await _makeHttpRequest(
        'POST',
        '$baseUrl/vocab/chapters/${chapter1['id']}/complete',
        headers: {'Authorization': 'Bearer $authToken'},
        body: {'finalScore': 95, 'completionNotes': 'QA Test completion'},
      );

      // Validate completion response
      if (!completeResponse['success']) {
        throw Exception(
          'Chapter completion failed: ${completeResponse['message']}',
        );
      }
      ...
      stdout.writeln('✅ Chapter 1 completed successfully');
      stdout.writeln(
        '   - Next chapter unlocked: ${completionData['nextChapterUnlocked']}',
      );
    } else {
      stdout.writeln(
        '   Chapter 1 already completed, skipping completion test',
      );
    }
    ...
    if (!finalChapter2['isUnlocked']) {
      throw Exception(
        'Chapter 2 should be unlocked after completing chapter 1',
      );
    }

    stdout.writeln('✅ Progressive unlocking validation passed');
    stdout.writeln('   Final state:');
    stdout.writeln('   - Chapter 1 completed: ${finalChapter1['isCompleted']}');
    stdout.writeln('   - Chapter 2 unlocked: ${finalChapter2['isUnlocked']}');
  }
  ...
  Future<void> _testEdgeCases() async {
    ...
    if (lockedChapter != null) {
      stdout.writeln(
        '   Testing access to locked chapter ${lockedChapter['order']}...',
      );
      ...
      try {
        ...
        stdout.writeln('✅ Correctly blocked access to locked chapter');
      }
    }

    // Test invalid chapter ID
    stdout.writeln('   Testing invalid chapter ID...');
    try {
      await _makeHttpRequest(
        'POST',
      ...
      stdout.writeln('✅ Correctly rejected invalid chapter ID');
    }

    // Test unauthorized access
    stdout.writeln('   Testing unauthorized access...');
    try {
      await _makeHttpRequest('GET', '$baseUrl/vocab/chapters');
      ...
      stdout.writeln('✅ Correctly requires authorization');
    }

    stdout.writeln('✅ Edge cases validation passed');
  }
  ...
  Future<void> _testFrontendBackendSync() async {
    ...
    for (final response in responses) {
      if (response['data']['totalChapters'] !=
          firstResponse['data']['totalChapters']) {
        throw Exception('Inconsistent total chapters count across requests');
      }

      if (response['data']['completedChapters'] !=
          firstResponse['data']['completedChapters']) {
        throw Exception(
          'Inconsistent completed chapters count across requests',
        );
      }
    }

    stdout.writeln('✅ Frontend/Backend synchronization validation passed');
    stdout.writeln(
      '   - All ${responses.length} concurrent requests returned consistent data',
    );
  }
  ...
  Future<void> _testLivesIntegration() async {
    ...
    stdout.writeln('   Current lives status:');
    stdout.writeln(
      '   - Lives available: ${livesResponse['data']['currentLives']}',
    );
    stdout.writeln(
      '   - Has lives: ${livesResponse['data']['hasLivesAvailable']}',
    );
    ...
    if (!chaptersResponse['success']) {
      throw Exception(
        'Chapters endpoint should work regardless of lives status',
      );
    }

    stdout.writeln('✅ Lives system integration validation passed');
    stdout.writeln('   - Chapters can be viewed regardless of lives status');
  }
  ...
  Future<void> _testSecurityValidation() async {
    ...
    stdout.writeln('   Testing rate limiting...');
    ...
    try {
      ...
      stdout.writeln(
        '✅ All rapid requests succeeded (rate limiting may not be strict)',
      );
    } catch (e) {
      stdout.writeln(
    ...
    stdout.writeln('   Testing malformed requests...');
    ...
    try {
      ...
      stdout.writeln('✅ Malformed request properly handled');
    }

    stdout.writeln('✅ Security validation passed');
  }
