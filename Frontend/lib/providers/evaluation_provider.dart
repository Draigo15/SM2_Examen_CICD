...
enum EvaluationState { initial, loading, loaded, evaluating, evaluated, error }
...
class EvaluationProvider with ChangeNotifier {
  ...
  int get passedEvaluations => _evaluations
      .where(
        (e) =>
            e.status == EvaluationStatus.passed ||
            e.status == EvaluationStatus.excellent,
      )
      .length;
  int get failedEvaluations =>
      _evaluations.where((e) => e.status == EvaluationStatus.failed).length;
  double get averageScore => _evaluations.isEmpty
      ? 0.0
      : _evaluations.map((e) => e.score).reduce((a, b) => a + b) /
            _evaluations.length;
  ...
  Future<void> evaluateChapter(
    ...
    try {
      final token = _authProvider.token;
      final response = await _apiService.evaluateChapter(
        chapterId,
        evaluationData,
        token: token,
      );
      ...
      if (response.success && response.data != null) {
        ...
        final existingIndex = _evaluations.indexWhere(
          (e) => e.chapterNumber == chapterId,
        );
        if (existingIndex != -1) {
  ...
  Future<void> getEvaluationHistory({int? userId}) async {
    ...
    try {
      ...
      final userIdToUse =
          userId ??
          (_authProvider.user?.id is int
              ? _authProvider.user!.id as int
              : null);
      final response = await _apiService.getEvaluationHistory(
        userIdToUse,
        token: token,
      );
  ...
  Future<void> _savePendingEvaluations() async {
    try {
      ...
      await prefs.setString(
        _pendingEvaluationsKey,
        json.encode(_pendingEvaluations),
      );
    } catch (e) {
